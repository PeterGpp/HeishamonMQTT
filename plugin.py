...
<plugin key="HeishamonMQTT" name="Heishamon MQTT" version="0.2.3">
    ...
    <params>
        ...
        <param field="Mode2" label="Power values (Watt) source" width="300px">
            <options>
                <option label="Extra topics (K/L-series, precise)" value="Extra" default="true"/>
                <option label="Main topics (legacy / older firmware)" value="Main"/>
            </options>
        </param>
...
# Mapping of HeishaMon "extra/" power topics (only published by heatpumps with
# the extra data block, e.g. K/L-series) to the existing Domoticz device names.
# On these heatpumps the "main/..." power topics become invalid (e.g. -200),
# so the extra/..._Extra topics should be used instead.
EXTRA_POWER_TOPICS = {
    "Heat_Power_Consumption_Extra": "Heat_Energy_Consumption",
    "Heat_Power_Production_Extra":  "Heat_Energy_Production",
    "Cool_Power_Consumption_Extra": "Cool_Energy_Consumption",
    "Cool_Power_Production_Extra":  "Cool_Energy_Production",
    "DHW_Power_Consumption_Extra":  "DHW_Energy_Consumption",
    "DHW_Power_Production_Extra":   "DHW_Energy_Production",
}
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if errmsg =="":
            try:
                ...
                # Choose whether Watt consumption/production devices are fed
                # from the (legacy) "main/..." topics or from the more precise
                # "extra/..._Extra" topics (K/L-series heatpumps only).
                self.powerSource = "Extra"
                if "Mode2" in Parameters and Parameters["Mode2"].strip() != "":
                    self.powerSource = Parameters["Mode2"].strip()
    ...
    def onMQTTPublish(self, topic, message): # process incoming MQTT statuses
        ...
        #------------------ EXTRA (precise Watt values, K/L-series) -----------
        #------------------------------------------------------------------------
        # MQTT message --> panasonic_heat_pump/extra/Heat_Power_Consumption_Extra
        if ( (mqttpath[0] == self.base_topic) and (mqttpath[1] == 'extra') ):
            extraname = mqttpath[2].strip()
            if extraname in EXTRA_POWER_TOPICS:
                if self.powerSource == "Extra":
                    unitname = EXTRA_POWER_TOPICS[extraname]
                    self.updatePowerDevice(unitname, message)
                else:
                    Domoticz.Debug("Ignoring " + topic + ", plugin is configured to use 'main' topics for power values")
                return True
        ...
        if ( (mqttpath[0] == self.base_topic) and (mqttpath[1] == 'main') ):
            ...
            # If the plugin is configured to use the extra/..._Extra topics for
            # power (Watt) values, ignore the (possibly invalid/bogus) main
            # power topics entirely so they cannot overwrite the good values.
            if (self.powerSource == "Extra") and (unitname in self.kWh_devices):
                return True
            ...
            if ( unitname in self.kWh_devices ):
                # Only reached when self.powerSource == "Main" (Extra source
                # returns earlier above), so this keeps the legacy behaviour
                # for heatpumps without an extra data block.
                self.updatePowerDevice(unitname, message)
                return True

            # ------------------ Speed | Pressue | Counter | -----------------------
            # -----------------------------------------------------------------------
            if ( (unitname in self.speed_devices) or (unitname in self.pressure_devices) or (unitname in self.counter_devices) or (unitname in self.kelvin_devices) or (unitname == "Compressor_Current") or (unitname == "Compressor_Freq") or ( unitname == "Pump_Flow") ):
                try:
                    mval = int(message)
                except:
                    mval = str(message).strip()
